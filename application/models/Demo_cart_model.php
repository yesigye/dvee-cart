...
class Demo_cart_model extends CI_Model {
	...
	function demo_update_loaded_cart_data()
	{
		...
		{
			$sql_where = array('id' => $column['id']);
		
			$query = $this->db->get_where('products', $sql_where);
	...
	/**
	 * save_order
	 * This example validates data posted from the checkout page and then saves it along with the current cart.
	 */
	function save_order()
	{
