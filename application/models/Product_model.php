...
class Product_model extends CI_Model
{
	...
	public function get_products($options = array())
	{
		...
		*/
		...
		if (isset($options['search']))
		{
			...
			$this->db->or_like('products.description', $options['search']);
		}
	...
	public function get_product_variants($product_id)
	{
		...
		$defaults = $this->get_product_defaults($product_id);
		...
		foreach ($variants as $key => $row)
		{
			...
			{
				$row->default = (in_array($item->attribute_description_id, $defaults)) ? true : false;
	...
	public function update_product_defaults($product_id, $option_id = null)
	{
		// Delete current relationships.
		$this->db->where('product_id', $product_id)->delete('product_option_defaults');

		if($option_id) {
			$rows = $this->db->get_where('product_option_groups', ['product_option_id' => $option_id])->result();	
		}

		foreach ($rows as $row) {
			$this->db->insert('product_option_defaults', array(
				'product_id' => $product_id,
				'attribute_description_id' => $row->attribute_description_id
			));
		}
