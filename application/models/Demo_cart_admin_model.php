...
class Demo_cart_admin_model extends CI_Model {
	...
	function demo_resave_order($order_number)
	{
		...
		if (isset($this->post_data['save']))
		{
			...
			redirect('admin/order_details/'.$order_number);
		}
		else
		{
	...
	function demo_insert_item_to_cart()
	{
		...
		foreach($this->input->post('insert_item') as $item_id => $item_post_data)
		{
			...
			{
				$query = $this->db->select('name, weight')
					->from('products')
					->where('id', $item_id)
					->get();
				...
				if ($query->num_rows() == 1)
				{
					...
					$insert_item_data[] = array(
						'id' => $item_id, 
						'name' => 'Example Database '.$item_db_data['name'],
	...
	function demo_update_location_types()
	{
		...
		else
		{
			...
			$this->data['message'] = validation_errors('<p class="alert alert-danger">', '</p>');
	...
	function demo_insert_location($location_type_id)
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			// Set a message to the CI flashdata so that it is available after the page redirect.
			$this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
			redirect('admin/locations/'.$location_type_id);
		}
		else
		{
			// Set validation errors and field name ids so that data can be repopulated for all rows.
	...
	function demo_insert_shipping_rate($shipping_id)
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			// Set a message to the CI flashdata so that it is available after the page redirect.
			$this->session->set_flashdata('message', $this->flexi_cart_admin->get_messages('admin'));
			redirect('admin/shipping_rates/'.$shipping_id);
		}
		else
		{
			// Set validation errors and field name ids so that data can be repopulated for all rows.
	...
	function demo_update_item_stock()
	{
		foreach($this->input->post('update') as $row)
		{
			...
			$sql_where_price = array('id' => $row['id']);
			
			$this->db->update('products', $sql_update_price, $sql_where_price);
	...
	function demo_insert_currency()
	{
		...
		foreach($this->input->post('insert') as $id => $row)
		{
			...
			$this->form_validation->set_rules('insert['.$id.'][exchange_rate]', ' ', 'required');
			$this->form_validation->set_rules('insert['.$id.'][symbol]', ' ', 'required');
			$this->form_validation->set_rules('insert['.$id.'][thousand]', ' ', 'required');
			$this->form_validation->set_rules('insert['.$id.'][decimal]', ' ', 'required');
	...
	function demo_insert_discount()
	{
		...
		// Set validation rules.
		$this->form_validation->set_rules('insert[type]', ' ', 'required');
		$this->form_validation->set_rules('insert[method]', ' ', 'required');
		$this->form_validation->set_rules('insert[usage_limit]', ' ', 'required');
		$this->form_validation->set_rules('insert[valid_date]', ' ', 'required');
		$this->form_validation->set_rules('insert[expire_date]', ' ', 'required');
		...
		{
			// Set validation errors.
			$this->data['message'] = 'Check the form for errors and try again.';
	...
	function demo_insert_discount_group()
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			{
				$this->data['message'] = '<p class="alert alert-danger">There are no items matching the submitted SQL WHERE statement.</p>';
				$this->data['validation_row_ids'] = $row_ids;
	...
	function demo_insert_discount_group_items($group_id)
	{
		...
		foreach($this->input->post('selected') as $item_id)
		{
			$sql_insert = array(
				$this->flexi_cart_admin->db_column('discount_group_items', 'group') => $group_id,
				$this->flexi_cart_admin->db_column('discount_group_items', 'item') => $item_id
			);
	...
	function demo_reward_point_summary($user_id = FALSE)
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			{
				$summary_data = $this->flexi_cart_admin->get_db_reward_point_summary($column['id']);
	...
	function demo_converted_reward_point_history($user_id)
	{
		...
		$points_converted_data = $this->flexi_cart_admin->get_db_voucher_query($sql_select, $sql_where)->result_array();
		...
		foreach($points_converted_data as $row => $data)
		{
			...
			$points_converted_data[$row]['reward_points'] = $this->flexi_cart_admin->get_db_converted_reward_points_query($sql_select, $sql_where)->result_array();
		}
	...
	function demo_update_config()
	{
		...
		$sql_update = array(
			$this->flexi_cart_admin->db_column('configuration', 'pagination_limit') => $data['pagination_limit'],
			$this->flexi_cart_admin->db_column('configuration', 'user_file_limit') => $data['user_file_limit'],
