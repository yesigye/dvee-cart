...
<?php $this->load->view('admin/templates/header', array(
	...
	'breadcrumbs' => array(
		...
		1 => array(
			'name'=>$current_order_data[$this->flexi_cart_admin->db_column('order_summary', 'order_number')],
			'link'=>'order_details/'.$order_number
		),
	)
)); ?>

<?php $this->load->view('admin/orders/orders_sub_header'); ?>
...
<?php echo form_open(current_url()); ?>
	...
	<?php if ($current_order_data[$this->flexi_cart_admin->db_column('order_status', 'cancelled')] == 1): ?>
		<div class="alert alert-warning">
			This order is currently 'Cancelled', if the order is resaved, the status will be automatically changed to the default order status.
		</div>
	<?php endif ?>

	<div class="modal" id="add-cart-items-modal">
			...
			<table>
				...
				?>
					<tr>
						...
						<td class="text-center">
							<input type="text" name="insert_item[<?php echo $item_id; ?>][item_price]" value="<?php echo $item['item_price'];?>" class="width_50 text-center validate_decimal"/>
						</td>
						<td class="text-center">
							<input type="text" name="insert_item[<?php echo $item_id; ?>][item_quantity]" value="0" class="width_50 text-center validate_integer"/>
						</td>
						<td class="text-center">
							<?php 
	...
	<fieldset>
		<div class="lead page-header">Updated Order Details</div>
		<table id="cart_items" class="table table-striped">
			<thead>
				<tr>
					<th>Item</th>
					<th>Price</th>
					<th title="Indicates the total quantity of items that are being ordered.">
						Quantity Ordered
					</th>
					<th title="Indicates the quantity of items that have been marked as 'shipped'. Shipped items activate their associated reward points.">
						Quantity Shipped
					</th>
					<th title="Indicates the quantity of items that have been marked as 'cancelled'. Cancelled items are returned to stock.">
						Quantity Cancelled
					</th>
					<th>Total</th>
				</tr>
			...
			?>
				<tr>
					...
					<td class="text-center">
						<input type="text" name="items[<?php echo $i;?>][price]" value="<?php echo $this->flexi_cart_admin->item_price($row['row_id'], FALSE, FALSE, TRUE);?>" class="width_50 text-center validate_decimal"/><br/>
					<?php 
					...
					<td class="text-center">
						<input type="text" name="items[<?php echo $i;?>][quantity]" value="<?php echo $row['quantity'];?>" maxlength="3" class="width_50 text-center validate_integer"/>
					</td>
			...
			?>
				<tr class="discount">
					<td colspan="6">
						...
						: <a href="<?php echo $base_url; ?>admin/unset_discount/<?php echo $this->flexi_cart_admin->item_discount_id($row['row_id']).'/'.$order_number;?>">Remove</a>
					</td>
				</tr>
			<?php } } } else { ?>
				<tr>
	...
	<div class="lead page-header">Cart Summary</div>
		...
		<tbody>
			...
			<tr>
				<td class="form-inline">
					Shipping Option : 
					<select name="shipping" class="form-control input-sm">
				...
				<tr class="discount">
					<th>
						...
						<?php if (! empty($discount['id'])) { ?>
							: <a href="<?php echo $base_url; ?>admin/unset_discount/<?php echo $discount['id'].'/'.$order_number; ?>">Remove</a>
						<?php } ?>
						</span>
					</th>
			...
			<?php } ?>
				<tr class="discount">
					...
					<td class="text-right"><?php echo $this->flexi_cart_admin->cart_savings_total();?></td>
				</tr>
			<?php } ?>
				...
				<tr class="surcharge">
					<th>
						<span class="pad_l_20">
							...
							: <a href="<?php echo $base_url; ?>admin/unset_surcharge/<?php echo $surcharge['id'].'/'.$order_number; ?>">Remove</a>
						</span>
					</th>
			...
			<?php } ?>
				<tr class="surcharge">
					...
					<td class="text-right"><?php echo $this->flexi_cart_admin->surcharge_total();?></td>
				</tr>
			<?php } ?>
				...
				<tr class="voucher">
					<th>
						<span class="pad_l_20">
							...
							: <a href="<?php echo $base_url; ?>admin/unset_discount/<?php echo $voucher['id'].'/'.$order_number; ?>">Remove</a>
						</span>
					</th>
			...
			<?php } ?>
				<tr class="voucher">
					...
					<td class="text-right"><?php echo $this->flexi_cart_admin->reward_voucher_total();?></td>
				</tr>
			<?php } ?>
		</tbody>
		<tfoot>
			<tr>
				...
				<td class="text-right"><?php echo $this->flexi_cart_admin->sub_total(TRUE, TRUE, TRUE);?></td>
			</tr>
			<tr>
				<th>
			...
			<tr class="grand_total">
				<th>Grand Total</th>
				<td class="text-right"><?php echo $this->flexi_cart_admin->total(TRUE, TRUE, TRUE);?></td>
			</tr>
		</tfoot>
	</table>
	
	<input type="submit" name="update_order[order]" value="Update Cart (Without Saving)" class="btn btn-primary"/>
	<input type="submit" name="update_order[save]" value="Re-Save Cart as Order" class="btn btn-primary"/>

	<div id="reward-voucher-modal" class="modal">
		<div class="lead page-header">Discounts / Reward Vouchers / Surcharges</div>
		<small>
			...
			<table>
				...
				<thead>
					<tr>
						...
						<th class="spacer_200 text-center tooltip_trigger" 
				...
				<tbody>
					<tr>
						...
						<td class="text-center">
							<input type="text" name="discount[0][value]" value="0" class="width_50 text-center validate_decimal"/>
			...
			<table>
				...
				<thead>
					<tr>
						...
						<th class="spacer_300 text-center tooltip_trigger" 
				...
				<tbody>
					<tr>
						...
						<td class="text-center">
							<input type="text" name="surcharge[0][tax_rate]" value="" placeholder="default" class="width_50 text-center validate_decimal"/>
						</td>
						<td class="text-center">
							<input type="text" name="surcharge[0][value]" value="0" class="width_50 text-center validate_decimal"/>
