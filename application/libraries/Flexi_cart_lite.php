...
class Flexi_cart_lite
{
	...
	{
		$this->load->model('flexi_cart_lite_model');
		$this->cart_contents = $this->flexi->cart_contents;
	}
	...
	{
		return (! empty($this->flexi->cart_contents['items']));
	}
	...
	{
		return (isset($this->flexi->cart_contents['summary'])) ? $this->flexi->cart_contents['summary'] : array();
	}
	...
	{
		$non_discounted_price_summary = $this->flexi->cart_contents['summary']['item_summary_total'];
		$discounted_price_summary = ($inc_discount) ? $this->flexi->cart_contents['settings']['discounts']['data']['item_discount_savings'] : 0;

		if (! $internal_value)
		{
			$non_discounted_tax_summary = $this->flexi->cart_contents['settings']['tax']['data']['item_total_tax'];		
			$non_discounted_price_summary = $this->convert_to_cart_currency($non_discounted_price_summary, $non_discounted_tax_summary);
			
			$discounted_tax_summary = ($inc_discount) ? $this->flexi->cart_contents['settings']['tax']['data']['item_discount_tax'] : 0;		
	...
	{		
		...
		$inc_discount = ($inc_discount && isset($this->flexi->cart_contents['settings']['discounts']['active_summary']['shipping_total']));
		
		$non_discounted_shipping_total = $this->flexi->cart_contents['summary']['shipping_total'];
		$discounted_shipping_total = ($inc_discount) ? $this->flexi->cart_contents['settings']['discounts']['active_summary']['shipping_total']['value'] : 0;					
		
		if (! $internal_value)
		{
			$non_discounted_shipping_tax = $this->flexi->cart_contents['settings']['tax']['data']['shipping_tax'];
			$non_discounted_shipping_total = $this->convert_to_cart_currency($non_discounted_shipping_total, $non_discounted_shipping_tax);
			
			$discounted_shipping_tax = ($inc_discount) ? $this->flexi->cart_contents['settings']['discounts']['active_summary']['shipping_total']['tax_value'] : 0;
	...
	public function tax_total($inc_discount = TRUE, $format = TRUE, $internal_value = FALSE)
	{
		...
		if ($inc_discount)
		{
			$tax_total = $this->flexi->cart_contents['settings']['tax']['data']['cart_tax'] + $this->flexi->cart_contents['settings']['tax']['data']['surcharge_tax'];
		}
		else
		{
			$tax_total = $this->flexi->cart_contents['summary']['tax_total'];
		}
	...
	public function sub_total($inc_discount = TRUE, $format = TRUE, $internal_value = FALSE)
	{
		if ($inc_discount)
		{
			...
			$cart_taxable_value = $this->flexi->cart_contents['settings']['tax']['data']['cart_taxable_value'];
			$cart_non_taxable_value = $this->flexi->cart_contents['settings']['tax']['data']['cart_non_taxable_value'];
			...
			$surcharge_total = $this->flexi->cart_contents['summary']['surcharge_total'];
		...
		else
		{
			...
			$surcharge_total = 0;
			$sub_total = ($this->flexi->cart_contents['summary']['total'] - $this->flexi->cart_contents['summary']['tax_total']);
		}
	...
	public function total($inc_discount = TRUE, $format = TRUE, $internal_value = FALSE)
	{
		if ($inc_discount)
		{
			...
			$cart_taxable_value = $this->flexi->cart_contents['settings']['tax']['data']['cart_taxable_value'];
			$cart_non_taxable_value = $this->flexi->cart_contents['settings']['tax']['data']['cart_non_taxable_value'];
			$cart_tax_value = $this->flexi->cart_contents['settings']['tax']['data']['cart_tax'];
		...
		else
		{
			// The non-discounted cart summary total is to be used, therefore the total already includes the surcharge value.
			$surcharge_total = 0;
			$total = $this->flexi->cart_contents['summary']['total'];
		}
			
		if (! $internal_value)
		{
			$surcharge_tax = ($inc_discount) ? $this->flexi->cart_contents['settings']['tax']['data']['surcharge_tax'] : 0;
	...
	{
		$item_summary_savings_total = $this->flexi->cart_contents['settings']['discounts']['data']['item_discount_savings'];
		
		if (! $internal_value)
		{
			$item_discount_tax_total = $this->flexi->cart_contents['settings']['tax']['data']['item_discount_tax'];
	...
	{
		$summary_savings_total = $this->flexi->cart_contents['settings']['discounts']['data']['summary_discount_savings'];
		
		if (! $internal_value)
		{
			$summary_discount_tax_total = $this->flexi->cart_contents['settings']['tax']['data']['summary_discount_tax'];
	...
	{
		$reward_voucher_total = $this->flexi->cart_contents['settings']['discounts']['data']['reward_vouchers'];
		
		if (! $internal_value)
		{
			$reward_voucher_tax_total = $this->flexi->cart_contents['settings']['tax']['data']['reward_voucher_tax'];
	...
	public function surcharge_total($format = TRUE, $internal_value = FALSE)
	{
		...
		if (! $internal_value)
		{
			$surcharge_tax_total = $this->flexi->cart_contents['settings']['tax']['data']['surcharge_tax'];
	...
	{
		return $this->format_number($this->flexi->cart_contents['summary']['total_rows'], $format, 0);
	}
	...
	{
		return $this->format_number($this->flexi->cart_contents['summary']['total_items'], $format, 
			$this->flexi->cart_contents['settings']['configuration']['quantity_decimals']);
	}
	...
	{
		return $this->convert_weight($this->flexi->cart_contents['summary']['total_weight'], FALSE, $weight_type, $format, $decimals);
	}
	...
	{
		return $this->format_number($this->flexi->cart_contents['summary']['total_reward_points'], $format, 0);
	}
	...
	{	
		...
		if (isset($this->flexi->cart_contents['items'][$row_id]))
		{
			...
			$column_alias = $this->flexi->cart_columns;
	...
	{
		return (isset($this->flexi->cart_contents['items'][$row_id])) ? 
			$this->flexi->cart_contents['items'][$row_id] : $this->flexi->cart_contents['items'];
	}	
	...
	{
		return (isset($this->flexi->cart_contents['items'][$row_id])) ?
			$this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_id']] : FALSE;
	}
	...
	{
		return (isset($this->flexi->cart_contents['items'][$row_id])) ?
			$this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_name']] : FALSE;
	}
	...
	public function item_quantity($row_id = FALSE, $inc_discount = FALSE, $format = TRUE)
	{
		if (isset($this->flexi->cart_contents['items'][$row_id]))
		{
			$item_quantity = ($inc_discount && $this->item_discount_status($row_id)) ? 
				$this->flexi->cart_contents['settings']['discounts']['active_items'][$row_id]['non_discount_quantity'] : 
				$this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_quantity']];
				
			return $this->format_number($item_quantity, $format, $this->flexi->cart_contents['settings']['configuration']['quantity_decimals']);
		}
	...
	public function item_price($row_id = FALSE, $inc_discount = FALSE, $format = TRUE, $internal_value = FALSE)
	{
		if (isset($this->flexi->cart_contents['items'][$row_id]))
		{
			$item_price = $this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_price']];
			
			if ($inc_discount && $this->item_discount_status($row_id))
			{
				$item_price -= $this->flexi->cart_contents['settings']['discounts']['active_items'][$row_id]['value'];
			}
	...
	public function item_tax_rate($row_id = FALSE, $format = TRUE, $fallback_default = FALSE)
	{
		if (isset($this->flexi->cart_contents['items'][$row_id]))
		{
			$item_tax_rate = $this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_tax_rate']];
			...
			return ($format && $item_tax_rate !== FALSE) ? $this->flexi_cart_lite_model->format_calculation($item_tax_rate, 4).'%' : $item_tax_rate;
		}
	...
	public function item_tax($row_id = FALSE, $inc_discount = FALSE, $format = TRUE, $internal_value = FALSE)
	{
		if (isset($this->flexi->cart_contents['items'][$row_id]))
		{
			$item_tax = ($inc_discount && $this->item_discount_status($row_id)) ?
				$this->flexi->cart_contents['settings']['discounts']['active_items'][$row_id]['tax_value'] :
				$this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_tax']];
	...
	{
		$item_tax_rate = $this->flexi_cart_lite_model->get_database_item_tax_rate($item_id);
	...
	public function get_item_tax_value($item_id = FALSE, $value = FALSE, $format = TRUE, $internal_value = FALSE)
	{
		...
		$item_tax_data = $this->flexi_cart_lite_model->calculate_tax($value, $item_tax_rate);		
	...
	{
		return (! in_array($row_id, $this->flexi->cart_contents['settings']['shipping']['data']['banned_shipping_items']));
	}
	...
	{
		if (isset($this->flexi->cart_contents['settings']['shipping']['data']['item_shipping_rates'][$row_id]))
		{			
			$item_shipping_rate = $this->flexi->cart_contents['settings']['shipping']['data']['item_shipping_rates'][$row_id];
	...
	{
		if ($item_shipping_data = $this->flexi_cart_lite_model->get_database_item_shipping_rate($item_id, $location))
		{			
	...
	{
		return $this->flexi_cart_lite_model->get_database_item_shipping_status($item_id);
	}
	...
	{
		$item_shipping_data = $this->flexi_cart_lite_model->get_database_item_shipping_rate($item_id);
		
		return ($item_shipping_data) ? (bool)$item_shipping_data[$this->flexi->cart_database['item_shipping']['columns']['separate']] : FALSE;			
	}
	...
	{
		return $this->flexi_cart_lite_model->is_positive($this->item_stock_quantity($row_id, $deduct_cart_quantity, $apply_quantity));
	}
	...
	public function item_stock_quantity($row_id = FALSE, $deduct_cart_quantity = FALSE, $apply_quantity = 0, $format = TRUE)
	{
		if (isset($this->flexi->cart_contents['items'][$row_id]))
		{
			$item_stock_quantity = ($this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_stock_quantity']]);
			...
			if ($item_stock_quantity !== FALSE)
			{
				...
				return $this->format_number($item_stock_quantity, $format, $this->flexi->cart_contents['settings']['configuration']['quantity_decimals']);
			}
		}
	...
	{
		return $this->flexi_cart_lite_model->is_positive($this->flexi_cart_lite_model->get_item_stock_quantity($item_id, $deduct_cart_quantity, $apply_quantity));
	}
	...
	{
		$item_stock_quantity = $this->flexi_cart_lite_model->get_item_stock_quantity($item_id);
		
		if ($item_stock_quantity !== FALSE)
		{
			$item_stock_quantity += ($deduct_cart_quantity) ? ($this->flexi_cart_lite_model->get_item_allocated_stock($item_id) + $apply_quantity) : $apply_quantity; 
	...
	{
		return (isset($this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_options']]) && 
			count($this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_options']]) > 0);
	}
	...
	public function item_options($row_id = FALSE, $include_keys = FALSE, $option_separator = ', ')
	{
		...
		{
			if (is_array($this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_options']]))
			{
				$item_options = NULL;
				foreach($this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_options']] as $option => $value)
				{
			...
			{
				$item_options = $this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_options']];
			}
	...
	public function item_options_array($row_id = FALSE)
	{
		return ($this->item_option_status($row_id)) ? 
			(array)$this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_options']] : array();
	}
	...
	public function item_reward_points($row_id = FALSE, $format = TRUE)
	{
		if (isset($this->flexi->cart_contents['items'][$row_id]))
		{
			$array_reward_points = $this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_reward_points']];
			...
			else if (in_array($row_id, $this->flexi->cart_contents['settings']['discounts']['data']['void_reward_point_items']) || 
	...
	public function item_weight($row_id = FALSE, $weight_type = FALSE, $format = TRUE, $decimals = FALSE)
	{
		if (isset($this->flexi->cart_contents['items'][$row_id]))
		{
			$item_weight = $this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_weight']];
	...
	public function item_discount_status($row_id = FALSE, $include_shipping_discount = FALSE)
	{
		...
		{
			return (isset($this->flexi->cart_contents['settings']['discounts']['active_items'][$row_id]) &&
				! $this->flexi->cart_contents['settings']['discounts']['active_items'][$row_id]['shipping_discount']);
		}
		else
		{
			return (isset($this->flexi->cart_contents['settings']['discounts']['active_items'][$row_id]));
	...
	public function item_discount_id($row_id = FALSE)
	{
		return ($this->item_discount_status($row_id)) ?
			$this->flexi->cart_contents['settings']['discounts']['active_items'][$row_id]['id'] : FALSE;
	}
	...
	{
		return ($this->item_discount_status($row_id)) ?
			$this->flexi->cart_contents['settings']['discounts']['active_items'][$row_id]['description'] : FALSE;
	}
	...
	public function item_non_discount_quantity($row_id = FALSE, $format = TRUE)
	{
		if (isset($this->flexi->cart_contents['items'][$row_id]))
		{
			...
			return $this->format_number($item_non_discount_quantity, $format, $this->flexi->cart_contents['settings']['configuration']['quantity_decimals']);
		}
	...
	public function item_discount_quantity($row_id = FALSE, $format = TRUE)
	{
		if (isset($this->flexi->cart_contents['items'][$row_id]))
		{
			$item_discount_quantity = ($this->item_discount_status($row_id)) ? 
				$this->flexi->cart_contents['settings']['discounts']['active_items'][$row_id]['discount_quantity'] : 0;
				
			return $this->format_number($item_discount_quantity, $format, $this->flexi->cart_contents['settings']['configuration']['quantity_decimals']);
		}
	...
	{
		if (isset($this->flexi->cart_contents['items'][$row_id]) && 
			count($this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_status_message']]) !== 0)
		{
			// Loop through each message adding a html line break after each.
			foreach($this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_status_message']] as $message)
			{
	...
	public function item_status_message_array($row_id = FALSE)
	{
		return (isset($this->flexi->cart_contents['items'][$row_id]) && 
			count($this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_status_message']]) > 0) ? 
			$this->flexi->cart_contents['items'][$row_id][$this->flexi->cart_columns['item_status_message']] : array();
	}
	...
	{
		if (isset($this->flexi->cart_contents['items'][$row_id][$column_name]))
		{
			$item_column = $this->flexi->cart_contents['items'][$row_id][$column_name];
	...
	{
		return (isset($this->flexi->cart_contents['settings']['admin_data']['items'][$row_id]));
	}
	...
	{
		$item_shipped_quantity = (isset($this->flexi->cart_contents['settings']['admin_data']['items'][$row_id])) ? 
			$this->flexi->cart_contents['settings']['admin_data']['items'][$row_id]['item_quantity_shipped'] : 0;
		
		return $this->format_number($item_shipped_quantity, $format, $this->flexi->cart_contents['settings']['configuration']['quantity_decimals']);
	}
	...
	{
		$item_cancelled_quantity = (isset($this->flexi->cart_contents['settings']['admin_data']['items'][$row_id])) ? 
			$this->flexi->cart_contents['settings']['admin_data']['items'][$row_id]['item_quantity_cancelled'] : 0;
		
		return $this->format_number($item_cancelled_quantity, $format, $this->flexi->cart_contents['settings']['configuration']['quantity_decimals']);
	}	
	...
	{
		return $this->flexi_cart_lite_model->locations_tiered();
	}
	...
	{
		return $this->flexi_cart_lite_model->locations_inline($separator);
	}
	...
	{
		return $this->flexi_cart_lite_model->location_zones($zone_type);
	}
	...
	{
		return $this->flexi->cart_contents['settings']['shipping']['id'];
	}
	...
	{
		return $this->flexi->cart_contents['settings']['shipping']['name'];
	}
	...
	{
		return $this->flexi->cart_contents['settings']['shipping']['description'];
	}
	...
	{
		return $this->flexi->cart_contents['settings']['shipping']['location'];
	}
	...
	{
		if ($this->flexi_cart_lite_model->is_positive($location_id))
		{
	...
	{
		if ($this->flexi_cart_lite_model->is_positive($location_type_id))
		{
			foreach($this->shipping_location_data() as $location)
			{
	...
	public function location_shipping_status($check_all_items_permitted = TRUE)
	{
		...
		{
			return (empty($this->flexi->cart_contents['settings']['shipping']['data']['banned_shipping_items']));		
		}
		else
		{
			return (count($this->flexi->cart_contents['settings']['shipping']['data']['banned_shipping_items']) != count($this->flexi->cart_contents['items']));
		}
	}
	...
	{
		return $this->flexi->cart_contents['settings']['tax']['name'];
	}
	...
	public function tax_rate($suffix_delimiter = '%', $internal_value = FALSE)
	{
		$tax_rate = (! $internal_value) ? 
			$this->flexi->cart_contents['settings']['tax']['rate'] : $this->flexi->cart_contents['settings']['tax']['internal_rate'];
	...
	{
		return $this->flexi->cart_contents['settings']['tax']['location'];
	}
	...
	{
		return (bool)$this->flexi->cart_contents['settings']['configuration']['display_tax_prices'];
	}
	...
	{
		return (bool)$this->flexi->cart_contents['settings']['configuration']['price_inc_tax'];
	}
	...
	private function get_active_discounts($discount_type = FALSE, $target = FALSE, $format = TRUE, $internal_value = FALSE)
	{
		if ($target)
		{
			if (isset($this->flexi->cart_contents['settings']['discounts'][$discount_type][$target]))
			{
				$target_discount = $this->flexi->cart_contents['settings']['discounts'][$discount_type][$target];
		...
		{
			$discount_data = array();
			foreach($this->flexi->cart_contents['settings']['discounts'][$discount_type] as $discount_id => $discount)
			{
	...
	{
		return (! empty($this->flexi->cart_contents['settings']['discounts']['active_items']) || 
			! empty($this->flexi->cart_contents['settings']['discounts']['active_summary']));
	}
	...
	{
		return (! empty($this->flexi->cart_contents['settings']['discounts']['active_summary']));
	}
	...
	{
		return (isset($this->flexi->cart_contents['settings']['discounts']['codes'])) ? 
			$this->flexi->cart_contents['settings']['discounts']['codes'] : FALSE;	
	}
	...
	{
		return $this->flexi->cart_contents['settings']['discounts']['data']['excluded_discounts'];
	}
	...
	{
		if ($discount_data = $this->flexi_cart_lite_model->get_discount_requirements($discount_id))
		{		
	...
	{
		if ($item_discount_data = $this->flexi_cart_lite_model->get_item_discounts($item_id, $sql_select, $sql_where))
		{
			// Loop through data and change true/false columns from integers to boolean.
			foreach($item_discount_data as $i => $discount_data)
			{
				if (isset($discount_data[$this->flexi->cart_database['discounts']['columns']['custom_status_1']]))
				{
					$item_discount_data[$i][$this->flexi->cart_database['discounts']['columns']['custom_status_1']] = 
						$discount_data[$this->flexi->cart_database['discounts']['columns']['custom_status_1']];
				}

				if (isset($discount_data[$this->flexi->cart_database['discounts']['columns']['custom_status_2']]))
				{
					$item_discount_data[$i][$this->flexi->cart_database['discounts']['columns']['custom_status_2']] = 
						$discount_data[$this->flexi->cart_database['discounts']['columns']['custom_status_2']];
				}

				if (isset($discount_data[$this->flexi->cart_database['discounts']['columns']['custom_status_3']]))
				{
					$item_discount_data[$i][$this->flexi->cart_database['discounts']['columns']['custom_status_3']] = 
						$discount_data[$this->flexi->cart_database['discounts']['columns']['custom_status_3']];
				}

				if (isset($discount_data[$this->flexi->cart_database['discounts']['columns']['non_combinable']]))
				{
					$item_discount_data[$i][$this->flexi->cart_database['discounts']['columns']['non_combinable']] = 
						(bool)$discount_data[$this->flexi->cart_database['discounts']['columns']['non_combinable']];
				}
			}
		}
	...
	{
		return $this->flexi_cart_lite_model->database_date_time(-time(), $expire_date, TRUE);
	}
	...
	{
		$description_data = $this->flexi->cart_contents['settings']['discounts']['active_items'];
	...
	{
		$description_data = $this->flexi->cart_contents['settings']['discounts']['active_summary'];
	...
	{
		return (! empty($this->flexi->cart_contents['settings']['discounts']['reward_vouchers']));
	}
	...
	{	
		$voucher_data = array();
		$i = 0;
		foreach($this->flexi->cart_contents['settings']['discounts']['reward_vouchers'] as $voucher)
		{
			if (! $internal_value)
			{
	...
	{
		$description_data = $this->flexi->cart_contents['settings']['discounts']['reward_vouchers'];
	...
	{
		return $this->flexi->cart_contents['settings']['configuration']['reward_point_multiplier'];
	}
	...
	{
		return round($value * $this->flexi->cart_contents['settings']['configuration']['reward_point_multiplier']);
	}
	...
	{
		return (! empty($this->flexi->cart_contents['settings']['surcharges']));
	}
	...
	public function surcharge_data($surcharge_id = FALSE, $format = TRUE, $internal_value = FALSE)
	{
		if ($surcharge_id)
		{
			if (isset($this->flexi->cart_contents['settings']['surcharges'][$surcharge_id]))
			{
				$target_surcharge = $this->flexi->cart_contents['settings']['surcharges'][$surcharge_id];
		...
		else
		{
			...
			foreach($this->flexi->cart_contents['settings']['surcharges'] as $surcharge_id => $surcharge)
			{
				if (! $internal_value)
				{
	...
	{
		$description_data = $this->flexi->cart_contents['settings']['surcharges'];
	...
	{
		return (! $internal_value) ? $this->flexi->cart_contents['settings']['currency']['name'] : 
			$this->flexi->cart_contents['settings']['currency']['default']['name'];
	}
	...
	public function get_currency_symbol($internal_value = FALSE, $currency_name = FALSE)
	{
		...
		{
			$sql_select = $this->flexi->cart_database['currency']['columns']['symbol'];
			...
			$currency_data = $this->flexi_cart_lite_model->get_currency_data($sql_select, $sql_where, TRUE);
			
			if (! empty($currency_data))
			{
				return $currency_data[$this->flexi->cart_database['currency']['columns']['symbol']];
			}
		}
		
		if ($internal_value)
		{
			return $this->flexi->cart_contents['settings']['currency']['default']['symbol'];
		}
		else
		{
			return $this->flexi->cart_contents['settings']['currency']['symbol'];
		}
	}
	...
	public function get_exchange_rate($currency_name = FALSE, $decimals = 4)
	{
		if ($currency_name)
		{
			$sql_select = $this->flexi->cart_database['currency']['columns']['exchange_rate'];
		...
		$exchange_rate = (! empty($currency_data)) ? 
			$currency_data[$this->flexi->cart_database['currency']['columns']['exchange_rate']] :
			$this->flexi->cart_contents['settings']['currency']['exchange_rate'];
	...
	{
		return $this->flexi_cart_lite_model->get_currency_data($sql_select, $sql_where, FALSE);
	}
	...
	private function convert_tax_value($value = 0, $tax_rate = FALSE, $inverse = FALSE)
	{
		...
		if ($this->cart_prices_inc_tax())
		{
			...
			$value = $this->flexi_cart_lite_model->add_remove_tax($value, $remove_tax_rate, FALSE);
		}
		...
		if ($this->display_prices_inc_tax())
		{
			...
			$value = $this->flexi_cart_lite_model->add_remove_tax($value, $tax_rate, TRUE);
		}
	...
	{
		return $this->flexi_cart_lite_model->format_value('currency', $value, $format, $decimals, $internal_format, $currency_name, FALSE); 
	}
	...
	{
		return $this->flexi->cart_contents['settings']['configuration']['cart_data_id'];
	}
	...
	{
		return $this->flexi->cart_contents['settings']['configuration']['order_number'];
	}
	...
	{
		$minimum_order_value = $this->flexi->cart_contents['settings']['configuration']['minimum_order'];
	...
	{
		return $this->flexi_cart_lite_model->database_date_time($apply_time, $date_time, $force_unix);
	}
	...
	{
		return $this->flexi_cart_lite_model->get_weight_types($name);
	}
	...
	{
		$weight_data = $this->flexi_cart_lite_model->get_weight_types($this->flexi->cart_contents['settings']['configuration']['weight_type']);
	...
	public function convert_weight($value = 0, $convert_from = FALSE, $convert_to = FALSE, $format = TRUE, $decimals = FALSE)
	{
		...
		$weight_data = $this->flexi_cart_lite_model->get_weight_types();
				
		// Get default weight data if not submitted.
		$convert_from = (! $convert_from) ? $this->flexi->cart_contents['settings']['configuration']['weight_type'] : $convert_from;
		$convert_to = (! $convert_to) ? $this->flexi->cart_contents['settings']['configuration']['weight_type'] : $convert_to;
		$decimals = ($decimals === FALSE) ? $this->flexi->cart_contents['settings']['configuration']['weight_decimals'] : $decimals;
	...
	{
		return $this->flexi_cart_lite_model->format_value('number', $value, $format, $decimals, $internal_format, $currency_name, FALSE); 
	}
	...
	public function format_weight($value = 0, $weight_type = FALSE, $format = TRUE, $decimals = FALSE, $internal_format = FALSE)
	{
		...
		return $this->flexi_cart_lite_model->format_value('weight', $value, $format, $decimals, $internal_format, FALSE, $weight_type); 
	}
	...
	{
		return $this->flexi->cart_contents['settings']['configuration']['custom_status_1'];
	}
	...
	{
		return $this->flexi->cart_contents['settings']['configuration']['custom_status_2'];
	}
	...
	{
		return $this->flexi->cart_contents['settings']['configuration']['custom_status_3'];
	}
	...
	public function db_table($table = FALSE)
	{
		...
		if (! isset($this->flexi->cart_database[$table]['table']) || ! $this->flexi->cart_database[$table]['table'])
		{
			return FALSE;
		}
		
		return $this->flexi->cart_database[$table]['table'];
	}
	...
	public function db_column($table = FALSE, $column = FALSE)
	{
		...
		if (! isset($this->flexi->cart_database[$table]['columns'][$column]) || ! $this->flexi->cart_database[$table]['columns'][$column])
		{
			return FALSE;
		}
		
		return $this->flexi->cart_database[$table]['columns'][$column];
	}
	...
	{
		return $this->flexi_cart_lite_model->set_status_message($status_message, $target_user, $overwrite_existing);
	}
	...
	{
		return $this->flexi_cart_lite_model->status_messages($target_user, $prefix_delimiter, $suffix_delimiter);
	}
	...
	{
		// echo $vars;
		return $this->flexi_cart_lite_model->set_error_message($error_message, $target_user, $overwrite_existing);
	}
	...
	{
		return $this->flexi_cart_lite_model->error_messages($target_user, $prefix_delimiter, $suffix_delimiter);
	}
	...
	{
		$messages['status'] = $this->flexi_cart_lite_model->status_messages($target_user, $prefix_delimiter, $suffix_delimiter);

		$messages['errors'] = $this->flexi_cart_lite_model->error_messages($target_user, $prefix_delimiter, $suffix_delimiter);
