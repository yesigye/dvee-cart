...
class Flexi_cart_admin extends Flexi_cart_lite
{
	public function __construct()
	{
		...
		$this->load->model('flexi_cart_admin_model');
	}
	...
	{
		return $this->flexi_cart_admin_model->get_table_data($table_name, $sql_select, $sql_where);
	}
	...
	{		
		return $this->flexi_cart_admin_model->insert_table_data($table_name, $sql_insert);
	}
	...
	{
		return $this->flexi_cart_admin_model->update_table_data($table_name, $sql_update, $sql_where);
	}
	...
	{
		return $this->flexi_cart_admin_model->delete_table_data($table_name, $sql_where);
	}
	...
	{
		$default_pk = $this->flexi->cart_database['location_type']['columns']['id'];
	
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['location_type']['table'], $sql_select, $sql_where, $default_pk, 'location');
	}	
	...
	{
		return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['location_type']['table'], $sql_insert, 'location');
	}
	...
	public function update_db_location_type($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['location_type']['table'], $sql_update, $sql_where, $default_pk, 'location');
	}
	...
	public function delete_db_location_type($sql_where = FALSE, $delete_children = FALSE)
	{
		...
		if ($this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['location_type']['table'], $sql_where, $default_pk, 'location'))
		{
			if (! empty($parent_ids))
			{
				...
				{
					$sql_where_locations = array($this->flexi->cart_database['locations']['columns']['type'] => $id_data[$default_pk]);			
	...
	{
		$default_pk = $this->flexi->cart_database['locations']['columns']['id'];
		
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['locations']['table'], $sql_select, $sql_where, $default_pk, 'location');
	}
	...
	{
		return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['locations']['table'], $sql_insert, 'location');
	}
	...
	public function update_db_location($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['locations']['table'], $sql_update, $sql_where, $default_pk, 'location');
	}
	...
	public function delete_db_location($sql_where = FALSE, $delete_children = FALSE)
	{
		...
		if ($this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['locations']['table'], $sql_where, $default_pk, 'location'))
		{
			if (! empty($parent_ids))
			{			
				...
				foreach($parent_ids as $id_data)
				{
					$sql_where_locations = array($this->flexi->cart_database['locations']['columns']['parent'] => $id_data[$default_pk]);			
					$this->delete_db_location($sql_where_locations, 'delete');

					$sql_where_discounts = array($this->flexi->cart_database['discounts']['columns']['location'] => $id_data[$default_pk]);			
					$this->delete_db_discount($sql_where_discounts);
					$this->delete_db_voucher($sql_where_discounts);

					$sql_where_shipping = array($this->flexi->cart_database['shipping_options']['columns']['location'] => $id_data[$default_pk]);			
					$this->delete_db_shipping($sql_where_shipping, 'delete');

					$sql_where_item_shipping = array($this->flexi->cart_database['item_shipping']['columns']['location'] => $id_data[$default_pk]);			
					$this->delete_db_item_shipping($sql_where_item_shipping);

					$sql_where_tax = array($this->flexi->cart_database['tax']['columns']['location'] => $id_data[$default_pk]);			
					$this->delete_db_tax($sql_where_tax);

					$sql_where_item_tax = array($this->flexi->cart_database['item_tax']['columns']['location'] => $id_data[$default_pk]);								
	...
	{
		$default_pk = $this->flexi->cart_database['location_zones']['columns']['id'];
		
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['location_zones']['table'], $sql_select, $sql_where, $default_pk, 'location');
	}	
	...
	{		
		return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['location_zones']['table'], $sql_insert, 'location');
	}
	...
	public function update_db_location_zone($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['location_zones']['table'], $sql_update, $sql_where, $default_pk, 'location');
	}
	...
	public function delete_db_location_zone($sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['location_zones']['table'], $sql_where, $default_pk, 'location');
	}
	...
	{
		$default_pk = $this->flexi->cart_database['shipping_options']['columns']['id'];
		
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['shipping_options']['table'], $sql_select, $sql_where, $default_pk, 'shipping');
	}	
	...
	public function insert_db_shipping($sql_insert = FALSE)
	{
		...
		if (isset($sql_insert[$this->flexi->cart_database['shipping_options']['columns']['tax_rate']]) && 
			$sql_insert[$this->flexi->cart_database['shipping_options']['columns']['tax_rate']] === '')
		{
			$sql_insert[$this->flexi->cart_database['shipping_options']['columns']['tax_rate']] = NULL;
		}

		return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['shipping_options']['table'], $sql_insert, 'shipping');
	}
	...
	public function update_db_shipping($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		if (isset($sql_update[$this->flexi->cart_database['shipping_options']['columns']['tax_rate']]) && 
			$sql_update[$this->flexi->cart_database['shipping_options']['columns']['tax_rate']] === '')
		{
			$sql_update[$this->flexi->cart_database['shipping_options']['columns']['tax_rate']] = NULL;
		}
		
		return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['shipping_options']['table'], $sql_update, $sql_where, $default_pk, 'shipping');
	}
	...
	public function delete_db_shipping($sql_where = FALSE, $delete_children = FALSE)
	{
		...
		if ($this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['shipping_options']['table'], $sql_where, $default_pk, 'shipping'))
		{				
			if (! empty($parent_ids))
			{
				...
				foreach($parent_ids as $id_data)
				{
					$sql_where_shipping_rates = array($this->flexi->cart_database['shipping_rates']['columns']['parent'] => $id_data[$default_pk]);			
	...
	{
		$default_pk = $this->flexi->cart_database['shipping_rates']['columns']['id'];
		
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['shipping_rates']['table'], $sql_select, $sql_where, $default_pk, 'shipping');
	}	
	...
	{		
		return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['shipping_rates']['table'], $sql_insert, 'shipping');
	}
	...
	public function update_db_shipping_rate($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['shipping_rates']['table'], $sql_update, $sql_where, $default_pk, 'shipping');
	}
	...
	public function delete_db_shipping_rate($sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['shipping_rates']['table'], $sql_where, $default_pk, 'shipping');
	}
	...
	{
		$default_pk = $this->flexi->cart_database['item_shipping']['columns']['id'];
		
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['item_shipping']['table'], $sql_select, $sql_where, $default_pk, 'item_shipping');
	}	
	...
	{		
		return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['item_shipping']['table'], $sql_insert, 'item_shipping');
	}
	...
	public function update_db_item_shipping($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['item_shipping']['table'], $sql_update, $sql_where, $default_pk, 'item_shipping');
	}
	...
	public function delete_db_item_shipping($sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['item_shipping']['table'], $sql_where, $default_pk, 'item_shipping');
	}
	...
	{
		$default_pk = $this->flexi->cart_database['tax']['columns']['id'];
		
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['tax']['table'], $sql_select, $sql_where, $default_pk, 'tax');
	}	
	...
	{
		return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['tax']['table'], $sql_insert, 'tax');
	}
	...
	public function update_db_tax($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['tax']['table'], $sql_update, $sql_where, $default_pk, 'tax');
	}
	...
	public function delete_db_tax($sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['tax']['table'], $sql_where, $default_pk, 'tax');
	}
	...
	{
		$default_pk = $this->flexi->cart_database['item_tax']['columns']['id'];
		
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['item_tax']['table'], $sql_select, $sql_where, $default_pk, 'item_tax');
	}	
	...
	{		
		return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['item_tax']['table'], $sql_insert, 'item_tax');
	}
	...
	public function update_db_item_tax($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['item_tax']['table'], $sql_update, $sql_where, $default_pk, 'item_tax');
	}
	...
	public function delete_db_item_tax($sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['item_tax']['table'], $sql_where, $default_pk, 'item_tax');
	}
	...
	{
		$default_pk = ($this->flexi->cart_database['item_stock']['columns']['id']) ? 
			$this->flexi->cart_database['item_stock']['columns']['id'] : $this->flexi->cart_database['item_stock']['columns']['item'];
		
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['item_stock']['table'], $sql_select, $sql_where, $default_pk, 'item_stock');
	}
	...
	{
		return $this->flexi_cart_admin_model->update_db_row_quantity('item_stock', $item_id, $stock_quantity, TRUE);
	}
	...
	{		
		return $this->flexi_cart_admin_model->update_db_row_quantity('item_stock', $item_id, $stock_quantity, FALSE);
	}
	...
	{		
		return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['item_stock']['table'], $sql_insert, 'item_stock');
	}
	...
	public function update_db_item_stock($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['item_stock']['table'], $sql_update, $sql_where, $default_pk, 'item_stock');
	}
	...
	public function delete_db_item_stock($sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['item_stock']['table'], $sql_where, $default_pk, 'item_stock');
	}
	...
	{
		$default_pk = $this->flexi->cart_database['currency']['columns']['id'];
		
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['currency']['table'], $sql_select, $sql_where, $default_pk, 'currency');
	}	
	...
	{		
		return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['currency']['table'], $sql_insert, 'currency');
	}
	...
	public function update_db_currency($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['currency']['table'], $sql_update, $sql_where, $default_pk, 'currency');
	}
	...
	public function delete_db_currency($sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['currency']['table'], $sql_where, $default_pk, 'currency');
	}
	...
	{
		$default_pk = $this->flexi->cart_database['order_status']['columns']['id'];
		
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['order_status']['table'], $sql_select, $sql_where, $default_pk, 'orders');
	}
	...
	{		
		return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['order_status']['table'], $sql_insert, 'orders');
	}
	...
	public function update_db_order_status($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['order_status']['table'], $sql_update, $sql_where, $default_pk, 'orders');
	}
	...
	public function delete_db_order_status($sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['order_status']['table'], $sql_where, $default_pk, 'orders');
	}
	...
	public function search_orders_query($search_query = FALSE, $exact_match = FALSE, $sql_select = FALSE, $sql_where = FALSE)
	{
		if (! empty($search_query))
		{
			...
			$order_summary_cols = $this->flexi->cart_database['order_summary']['search_order_cols'];

			$this->flexi_cart_admin_model->create_sql_like($order_summary_cols, $search_query, $exact_match);
		}
	...
	public function get_db_order_query($sql_select = FALSE, $sql_where = FALSE)
	{
		if ($this->flexi_cart_admin_model->get_enabled_status('orders'))
		{
			...
			$this->db->join($this->flexi->cart_database['order_details']['table'], 
				$this->flexi->cart_database['order_summary']['columns']['order_number'].' = '.$this->flexi->cart_database['order_details']['columns']['order_number']);
			
			$this->db->join($this->flexi->cart_database['order_status']['table'], 
				$this->flexi->cart_database['order_summary']['columns']['status'].' = '.$this->flexi->cart_database['order_status']['columns']['id']);
			
			$this->db->group_by($this->flexi->cart_database['order_summary']['columns']['order_number']);
		
			return $this->flexi_cart_admin_model
				->get_table_data($this->flexi->cart_database['order_summary']['table'], $sql_select, $sql_where, FALSE, 'orders');
		}
	...
	{
		$default_pk = $this->flexi->cart_database['order_details']['columns']['id'];
		
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['order_details']['table'], $sql_select, $sql_where, $default_pk, 'orders');
	}
	...
	{
		return $this->flexi_cart_admin_model->get_refund_itemised($order_number);
	}
	...
	{
		return $this->flexi_cart_admin_model->get_refund_summary($order_number);
	}
	...
	{
		return $this->flexi_cart_admin_model->update_order_summary($sql_update, $sql_where);
	}
	...
	{
		return $this->flexi_cart_admin_model->update_order_details($sql_update, $sql_where);
	}
	...
	public function delete_db_order($order_number = FALSE)
	{
		...
		if ($this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['order_summary']['table'], $sql_where, FALSE, 'orders'))
		{
			...
			return $this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['order_details']['table'], $sql_where, FALSE, 'orders');
		}
	...
	{
		if ($order_number = $this->flexi_cart_admin_model->generate_order_number($prefix = FALSE, $suffix = FALSE))
		{
	...
	{
		return $this->flexi_cart_admin_model->check_order_number_available($order_number);
	}
	...
	public function save_order($custom_summary_data = FALSE, $custom_item_data = FALSE, $order_number = FALSE)
	{
		...
		if (! $this->flexi_cart_admin_model->get_enabled_status('orders', TRUE))
		{
			return FALSE;
		}
		
		if ($this->flexi_cart_admin_model->save_order($custom_summary_data, $custom_item_data, $order_number))
		{
			$this->flexi_cart_admin_model->set_status_message('cart_order_save_successful', 'config');
			return TRUE;
		}
		else 
		{
			$this->flexi_cart_admin_model->set_error_message('cart_order_save_unsuccessful', 'config');
			return FALSE;
		}
	}
	...
	public function resave_order($summary_data = FALSE, $item_data = FALSE)
	{
		...
		if ($this->flexi_cart_admin_model->resave_order($summary_data, $item_data))
		{
			$this->flexi_cart_admin_model->set_status_message('cart_order_save_successful', 'config');
			return TRUE;
		}
	...
	public function email_order($order_number = FALSE, $email_to = FALSE, $email_subject = FALSE, $custom_data = FALSE)
	{
		...
		if ($this->flexi_cart_admin_model->email_order($order_number, $email_to, $email_subject, $custom_data))
		{
			$this->flexi_cart_admin_model->set_status_message('send_email_successful', 'config');
			return TRUE;
		}
		else 
		{
			$this->flexi_cart_admin_model->set_error_message('send_email_unsuccessful', 'config');
			return FALSE;
		}
	}	
	...
	{
		return $this->flexi_cart_admin_model->check_code_available($code, $omit_discount_id);
	}
	...
	{
		if ($this->flexi_cart_admin_model->get_enabled_status('discounts'))
		{
			$default_pk = $this->flexi->cart_database['discounts']['columns']['id'];

			$this->db->where($this->flexi->cart_database['discounts']['columns']['type'].' != 3');

			return $this->flexi_cart_admin_model
				->get_table_data($this->flexi->cart_database['discounts']['table'], $sql_select, $sql_where, $default_pk, 'discounts');
		}
	...
	{
		return $this->flexi_cart_admin_model->update_db_row_quantity('discount', $discount_id, $usage_quantity, TRUE);
	}
	...
	{		
		return $this->flexi_cart_admin_model->update_db_row_quantity('discount', $discount_id, $usage_quantity, FALSE);
	}
	...
	public function insert_db_discount($sql_insert = FALSE)
	{
		...
		if (isset($sql_insert[$this->flexi->cart_database['discounts']['columns']['code']]) && ! empty($sql_insert[$this->flexi->cart_database['discounts']['columns']['code']]))
		{
			if (! $this->flexi_cart_admin_model->check_code_available($sql_insert[$this->flexi->cart_database['discounts']['columns']['code']]))
			{
				$this->flexi_cart_admin_model->set_error_message('duplicate_discount_code', 'config');
				return FALSE;
			}
		}
	
		return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['discounts']['table'], $sql_insert, 'discounts');
	}
	...
	public function update_db_discount($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		{
			// If a discount code is set, check it is unique.
			if (isset($sql_update[$this->flexi->cart_database['discounts']['columns']['code']]) && ! empty($sql_update[$this->flexi->cart_database['discounts']['columns']['code']]))
			{
				$table_row_id = (is_array($sql_where)) ? $sql_where[$this->flexi->cart_database['discounts']['columns']['id']] : $sql_where;
				$code = $sql_update[$this->flexi->cart_database['discounts']['columns']['code']];
			
				if (! $this->flexi_cart_admin_model->check_code_available($code, $table_row_id))
				{
					$this->flexi_cart_admin_model->set_error_message('duplicate_discount_code', 'config');
					return FALSE;
				}
			...
			return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['discounts']['table'], $sql_update, $sql_where, $default_pk, 'discounts');
		}
	...
	public function delete_db_discount($sql_where = FALSE)
	{
		if ($this->flexi_cart_admin_model->get_enabled_status('discounts'))
		{
			...
			return $this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['discounts']['table'], $sql_where, $default_pk, 'discounts');
		}
	...
	{
		$default_pk = $this->flexi->cart_database['discount_groups']['columns']['id'];

		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['discount_groups']['table'], $sql_select, $sql_where, $default_pk, 'discounts');
	}
	...
	{		
		return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['discount_groups']['table'], $sql_insert, 'discounts');
	}
	...
	public function update_db_discount_group($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['discount_groups']['table'], $sql_update, $sql_where, $default_pk, 'discounts');
	}
	...
	public function delete_db_discount_group($sql_where = FALSE, $delete_children = FALSE)
	{
		...
		if ($this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['discount_groups']['table'], $sql_where, $default_pk, 'discounts'))
		{
			if (! empty($parent_ids))
			{
				...
				foreach($parent_ids as $id_data)
				{
					$sql_where_discount_group_items = array($this->flexi->cart_database['discount_group_items']['columns']['group'] => $id_data[$default_pk]);			
	...
	{
		return $this->flexi_cart_admin_model->check_item_in_discount_group($group_id, $item_id);
	}
	...
	{
		$default_pk = $this->flexi->cart_database['discount_group_items']['columns']['id'];

		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['discount_group_items']['table'], $sql_select, $sql_where, $default_pk, 'discounts');
	}
	...
	{
		if (! empty($sql_insert[$this->flexi->cart_database['discount_group_items']['columns']['group']]) && 
			! empty($sql_insert[$this->flexi->cart_database['discount_group_items']['columns']['item']]))
		{
			$group_id = $sql_insert[$this->flexi->cart_database['discount_group_items']['columns']['group']];
			$item_id = $sql_insert[$this->flexi->cart_database['discount_group_items']['columns']['item']];
			
			if (! $this->flexi_cart_admin_model->check_item_in_discount_group($group_id, $item_id))
			{
				return $this->flexi_cart_admin_model->insert_table_data($this->flexi->cart_database['discount_group_items']['table'], $sql_insert, 'discounts');
			}
		}
	...
	public function update_db_discount_group_item($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['discount_group_items']['table'], $sql_update, $sql_where, $default_pk, 'discounts');
	}
	...
	public function delete_db_discount_group_item($sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['discount_group_items']['table'], $sql_where, $default_pk, 'discounts');
	}
	...
	{
		$default_pk = $this->flexi->cart_database['discount_types']['columns']['id'];

		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['discount_types']['table'], $sql_select, $sql_where, $default_pk, 'discounts');
	}
	...
	{
		$default_pk = $this->flexi->cart_database['discount_methods']['columns']['id'];

		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['discount_methods']['table'], $sql_select, $sql_where, $default_pk, 'discounts');
	}
	...
	{
		$default_pk = $this->flexi->cart_database['discount_tax_methods']['columns']['id'];

		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['discount_tax_methods']['table'], $sql_select, $sql_where, $default_pk, 'discounts');
	}
	...
	{
		return $this->flexi_cart_admin_model->get_user_reward_points($sql_select, $sql_where);
	}
	...
	public function get_db_reward_point_summary($sql_where = FALSE)
	{
		...
		{
			$sql_where = array($this->flexi->cart_database['reward_points']['columns']['user'] => $sql_where);
		}

		return $this->flexi_cart_admin_model->get_user_reward_point_summary($sql_where);
	}
	...
	{
		if ($this->flexi_cart_admin_model->get_enabled_status('rewards'))
		{
			$default_pk = $this->flexi->cart_database['reward_points_converted']['columns']['id'];

			$this->db->join($this->flexi->cart_database['discounts']['table'], 
				$this->flexi->cart_database['reward_points_converted']['columns']['discount'].' = '.$this->flexi->cart_database['discounts']['columns']['id']);
				
			$this->db->join($this->flexi->cart_database['reward_points']['join_table'], 
				$this->flexi->cart_database['reward_points_converted']['columns']['reward'].' = '.$this->flexi->cart_database['reward_points']['columns']['id']);

			$this->db->join($this->flexi->cart_database['reward_points']['table'], $this->flexi->cart_database['reward_points']['join']);

			$this->db->order_by($this->flexi->cart_database['reward_points_converted']['columns']['date'], 
				$this->flexi->cart_database['reward_points']['columns']['order_date']);
			
			return $this->flexi_cart_admin_model
				->get_table_data($this->flexi->cart_database['reward_points_converted']['table'], $sql_select, $sql_where, $default_pk, 'rewards');
		}
	...
	{		
		return $this->flexi_cart_admin_model->get_reward_point_conversion_tiers($reward_points, $points_limit);
	}
	...
	{
		return $this->flexi_cart_admin_model->calculate_conversion_reward_points($reward_points, $points_limit);
	}
	...
	{
		return $this->flexi_cart_admin_model->calculate_reward_point_value($reward_points);
	}
	...
	public function get_db_voucher_query($sql_select = FALSE, $sql_where = FALSE)
	{
		if ($this->flexi_cart_admin_model->get_enabled_status('rewards'))
		{
			...
			return $this->flexi_cart_admin_model
				->get_table_data($this->flexi->cart_database['discounts']['table'], $sql_select, $sql_where, $default_pk, 'rewards');
		}
	...
	public function insert_db_voucher($user_id = FALSE, $points_to_convert = FALSE, $code = FALSE, $description = FALSE, $expire_days = FALSE)
	{
		...
		if ($code)
		{
			if (! $this->flexi_cart_admin_model->check_code_available($code))
			{
				$this->flexi_cart_admin_model->set_error_message('duplicate_voucher_code', 'config');
				return FALSE;
			}
		}
	
		return $this->flexi_cart_admin_model->insert_voucher($user_id, $points_to_convert, $code, $description, $expire_days);
	}
	...
	public function update_db_voucher($sql_update = FALSE, $sql_where = FALSE)
	{
		if ($this->flexi_cart_admin_model->get_enabled_status('rewards'))
		{
			...
			if (isset($sql_update[$this->flexi->cart_database['discounts']['columns']['code']]) && ! empty($sql_update[$this->flexi->cart_database['discounts']['columns']['code']]))
			{
				$table_row_id = $sql_where[$this->flexi->cart_database['discounts']['columns']['id']];
				$code = $sql_update[$this->flexi->cart_database['discounts']['columns']['code']];
			...
			return $this->flexi_cart_admin_model->update_table_data($this->flexi->cart_database['discounts']['table'], $sql_update, $sql_where, $default_pk, 'rewards');
		}
	...
	public function delete_db_voucher($sql_where = FALSE)
	{
		if ($this->flexi_cart_admin_model->get_enabled_status('rewards'))
		{
			...
			return $this->flexi_cart_admin_model->delete_table_data($this->flexi->cart_database['discounts']['table'], $sql_where, $default_pk, 'rewards');
		}
	...
	{		
		if ($this->flexi->cart_database['configuration']['columns']['id'])
		{
			$default_pk = $this->flexi->cart_database['configuration']['columns']['id'];
		...
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['configuration']['table'], $sql_select, $sql_where, $default_pk, 'config');
	}
	...
	public function update_db_config($sql_update = FALSE, $sql_where = FALSE)
	{
		...
		return $this->flexi_cart_admin_model
			->update_table_data($this->flexi->cart_database['configuration']['table'], $sql_update, $sql_where, $default_pk, 'config');
	}
	...
	{
		$default_pk = $this->flexi->cart_database['db_cart_data']['columns']['id'];
		
		return $this->flexi_cart_admin_model
			->get_table_data($this->flexi->cart_database['db_cart_data']['table'], $sql_select, $sql_where, $default_pk, 'db_cart_data');
	}
	...
	{
		return $this->flexi_cart_admin_model->unserialize_cart_data($sql_where);
	}
	...
	{
		if ($cart_data_id = $this->flexi_cart_admin_model->save_cart_data($user_id, $set_readonly_status, $sql_update, $force_overwrite))
		{
			$this->flexi_cart_admin_model->set_status_message('cart_data_save_successful', 'config');
			return $cart_data_id;
		}
		else 
		{
			$this->flexi_cart_admin_model->set_error_message('cart_data_save_unsuccessful', 'config');
			return FALSE;
		}
	}
	...
	public function load_cart_data($sql_where = FALSE, $set_admin_data = FALSE, $reset_locations = FALSE, $reset_discounts = FALSE, $reset_surcharges = FALSE)
	{
		...
		if ($this->flexi_cart_admin_model->get_enabled_status('db_cart_data'))
		{
			$cart_data = $this->flexi_cart_admin_model->unserialize_cart_data($sql_where);
			if (! empty($cart_data))
			{		
				...
				if ($set_admin_data)
				{
					$admin_data = $this->flexi_cart_admin_model->set_cart_admin_data($cart_data);		
					
					$this->flexi->cart_contents['settings']['admin_data']['items'] = $admin_data['items'];
					$this->flexi->cart_contents['settings']['admin_data']['discounts'] = $admin_data['discounts'];
				}
				
				// Check if any cart data is available to set to the cart array.
				if (empty($cart_data))
				{
					$this->flexi_cart_admin_model->set_error_message('cart_data_load_unsuccessful', 'config');
					return FALSE;
				}
				...
				{
					$this->flexi->cart_contents['items'] = $cart_data['items'];
				}
				
				// Set shipping location to cart array.
				if (isset($cart_data['settings']['shipping']['location']) && ! $reset_locations)
				{
					$this->flexi->cart_contents['settings']['shipping']['location'] = $cart_data['settings']['shipping']['location'];
				}
				
				// Set tax location to cart array.
				if (isset($cart_data['settings']['tax']['location']) && ! $reset_locations)
				{
					$this->flexi->cart_contents['settings']['tax']['location'] = $cart_data['settings']['tax']['location'];
				}
				
				// Set discount codes to cart array.
				if (isset($cart_data['settings']['discounts']['codes']) && ! $reset_discounts)
				{
					$this->flexi->cart_contents['settings']['discounts']['codes'] = $cart_data['settings']['discounts']['codes'];
				}
				
				// Set manually set discounts to cart array.
				if (isset($cart_data['settings']['discounts']['manual']) && ! $reset_discounts)
				{
					$this->flexi->cart_contents['settings']['discounts']['manual'] = $cart_data['settings']['discounts']['manual'];
				}
				
				// Set manually set discounts to cart array.
				if (isset($cart_data['settings']['surcharges']) && ! $reset_surcharges)
				{
					$this->flexi->cart_contents['settings']['surcharges'] = $cart_data['settings']['surcharges'];
				}
				
				// Set cart id to cart array.
				if (isset($cart_data['settings']['configuration']['cart_data_id']))
				{
					$this->flexi->cart_contents['settings']['configuration']['cart_data_id'] = $cart_data['settings']['configuration']['cart_data_id'];
				}
				
				// Set order number to cart array.
				if (isset($cart_data['settings']['configuration']['order_number']))
				{
					$this->flexi->cart_contents['settings']['configuration']['order_number'] = $cart_data['settings']['configuration']['order_number'];
				}
				...
				$this->flexi_cart->update_shipping($cart_data['settings']['shipping']['id'], FALSE, FALSE);
				$this->flexi_cart->update_tax(FALSE);

				// Recalculate cart.
				$this->flexi_cart_model->calculate_cart();
				...
				$this->flexi_cart_admin_model->set_status_message('cart_data_load_successful', 'config');		
				$this->session->set_userdata(array($this->flexi->cart['name'] => $this->flexi->cart_contents));
	...
	{
		if ($this->flexi_cart_admin_model->delete_cart_data($sql_where, $force_delete))
		{
			$this->flexi_cart_admin_model->set_status_message('cart_data_delete_successful', 'config');
			return TRUE;
		}
		else 
		{
			$this->flexi_cart_admin_model->set_error_message('cart_data_delete_unsuccessful', 'config');
			return FALSE;
		}
	}
	...
	{
		$this->flexi_cart_admin_model->create_sql_where($column_name, $comparison_operator, $value, $logic_operator);
	}
	...
	{
		$this->flexi_cart_admin_model->set_sql_to_var('select', $columns, FALSE, FALSE, $overwrite_existing);
	}
	
	public function sql_where($column = FALSE, $value = FALSE, $overwrite_existing = FALSE) 
	{
		$this->flexi_cart_admin_model->set_sql_to_var('where', $column, $value, FALSE, $overwrite_existing);
	}
	
	public function sql_or_where($column = FALSE, $value = FALSE, $overwrite_existing = FALSE) 
	{
		$this->flexi_cart_admin_model->set_sql_to_var('or_where', $column, $value, FALSE, $overwrite_existing);
	}
	
	public function sql_where_in($column = FALSE, $value = FALSE, $overwrite_existing = FALSE) 
	{
		$this->flexi_cart_admin_model->set_sql_to_var('where_in', $column, $value, FALSE, $overwrite_existing);
	}
	
	public function sql_or_where_in($column = FALSE, $value = FALSE, $overwrite_existing = FALSE) 
	{
		$this->flexi_cart_admin_model->set_sql_to_var('or_where_in', $column, $value, FALSE, $overwrite_existing);
	}
	
	public function sql_where_not_in($column = FALSE, $value = FALSE, $overwrite_existing = FALSE) 
	{
		$this->flexi_cart_admin_model->set_sql_to_var('where_not_in', $column, $value, FALSE, $overwrite_existing);
	}
	
	public function sql_or_where_not_in($column = FALSE, $value = FALSE, $overwrite_existing = FALSE) 
	{
		$this->flexi_cart_admin_model->set_sql_to_var('or_where_not_in', $column, $value, FALSE, $overwrite_existing);
	}
	
	public function sql_like($column = FALSE, $value = FALSE, $wildcard_position = 'both', $overwrite_existing = FALSE)
	{
		$this->flexi_cart_admin_model->set_sql_to_var('like', $column, $value, $wildcard_position, $overwrite_existing);
	}
	
	public function sql_or_like($column = FALSE, $value = FALSE, $wildcard_position = 'both', $overwrite_existing = FALSE)
	{
		$this->flexi_cart_admin_model->set_sql_to_var('or_like', $column, $value, $wildcard_position, $overwrite_existing);
	}
	
	public function sql_not_like($column = FALSE, $value = FALSE, $wildcard_position = 'both', $overwrite_existing = FALSE)
	{
		$this->flexi_cart_admin_model->set_sql_to_var('not_like', $column, $value, $wildcard_position, $overwrite_existing);
	}
	
	public function sql_or_not_like($column = FALSE, $value = FALSE, $wildcard_position = 'both', $overwrite_existing = FALSE)
	{
		$this->flexi_cart_admin_model->set_sql_to_var('or_not_like', $column, $value, $wildcard_position, $overwrite_existing);
	}
	
	public function sql_join($column = FALSE, $join_on = FALSE, $join_type = FALSE, $overwrite_existing = FALSE)
	{
		$this->flexi_cart_admin_model->set_sql_to_var('join', $column, $join_on, $join_type, $overwrite_existing);
	}
	
	public function sql_order_by($column = FALSE, $sort_direction = FALSE, $overwrite_existing = FALSE) 
	{
		$this->flexi_cart_admin_model->set_sql_to_var('order_by', $column, $sort_direction, FALSE, $overwrite_existing);
	}
	
	public function sql_group_by($columns = FALSE, $overwrite_existing = FALSE) 
	{
		$this->flexi_cart_admin_model->set_sql_to_var('group_by', $columns, FALSE, FALSE, $overwrite_existing);
	}
	
	public function sql_limit($limit = FALSE, $offset = FALSE, $overwrite_existing = FALSE) 
	{
		$this->flexi_cart_admin_model->set_sql_to_var('limit', $limit, $offset, $overwrite_existing);
	}
	...
	{
		$this->flexi_cart_admin_model->clear_arg_sql();
	}
}
