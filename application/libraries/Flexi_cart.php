...
class Flexi_cart extends Flexi_cart_lite
{
	...
	public function insert_items($item_data = FALSE, $update_existing_items = FALSE)
	{
		...
		{
			$this->flexi_cart_model->set_error_message('invalid_data', 'config');
			return FALSE;
		}
		...
		if (isset($item_data[$this->flexi->cart_columns['item_id']]))
		{
			$recalculate_cart = ($this->flexi_cart_model->insert_item($item_data, $update_existing_items)) ? TRUE : $recalculate_cart;
		}
		...
		else
		{
			...
			{
				if (is_array($value) && isset($value[$this->flexi->cart_columns['item_id']]))
				{
					$recalculate_cart = ($this->flexi_cart_model->insert_item($value, $update_existing_items)) ? TRUE : $recalculate_cart;

					// If an error message has been set, stop any further loops.
					if ($this->flexi_cart_model->error_messages())
					{
						break;
					}
				}
		...
		$this->flexi_cart_model->set_error_message('no_items_added', 'config');
		return FALSE;
	}
	...
	public function update_cart($item_data = FALSE, $settings_data = FALSE, $force_column_update = FALSE, $force_recalculate = FALSE)
	{
		...
		if (is_array($item_data) && ! empty($item_data))
		{
			...
			{
				$recalculate_cart = ($this->flexi_cart_model->update_item($item_data, $force_column_update)) ? TRUE : $recalculate_cart;
			}
			...
			else
			{
				...
				{
					if (is_array($value) && isset($value[$this->flexi->cart_columns['row_id']]))
					{
						$recalculate_cart = ($this->flexi_cart_model->update_item($value, $force_column_update)) ? TRUE : $recalculate_cart;
		...
		$this->flexi_cart_model->set_error_message('no_items_updated', 'config');
		return FALSE;
	}
	...
	public function delete_items($row_ids = FALSE)
	{
		...
		if (!is_array($row_ids))
		{
			unset($this->flexi->cart_contents['items'][$row_ids]);
		}
		else
		{
	...
	{
		return $this->flexi_cart_model->get_location_query($sql_select, $location_type_id, 'shipping_location_id');
	}
	...
	{
		return $this->flexi_cart_model->get_location_query($sql_select, $location_type_id, 'tax_location_id');
	}
	...
	public function update_shipping_location($locations = FALSE, $update_shipping = TRUE, $recalculate_cart = TRUE)
	{
		...
		if (! $this->flexi_cart_model->get_enabled_status('location', TRUE))
		{
			return FALSE;
		}

		// Compares submitted locations with database and then returns a cart formatted array of database location data.
		if ($shipping_location = $this->flexi_cart_model->get_database_location_data($locations, 'shipping'))
		{
			...
			if ($shipping_location != $this->shipping_location_data())
			{
				...
				$this->flexi->cart_contents['settings']['shipping']['location'] = $shipping_location;
		...
		$this->flexi_cart_model->set_status_message('shipping_location_update_unsuccessful', 'config');
		return FALSE;
	}
	...
	public function update_tax_location($locations = FALSE, $update_tax = TRUE, $recalculate_cart = TRUE)
	{
		...
		// Compares submitted locations with database and then returns a cart formatted array of database location data.
		if ($tax_location = $this->flexi_cart_model->get_database_location_data($locations, 'tax'))
		{
			// If returned tax location is different from cart session.
			if ($tax_location != $this->tax_location_data())
			{
				// Update cart session data.
				$this->flexi->cart_contents['settings']['tax']['location'] = $tax_location;
		...
		$this->flexi_cart_model->set_status_message('tax_location_update_unsuccessful', 'config');
		return FALSE;
	}
	...
	public function update_shipping($shipping_id = FALSE, $location = FALSE, $recalculate_cart = TRUE)
	{
		...
		if (! $this->flexi_cart_model->get_enabled_status('shipping', TRUE))
		{
			return FALSE;
		}
		
		// Update shipping id if submitted to function, else get the current shipping id.
		$shipping_id = ($this->flexi_cart_model->non_negative($shipping_id)) ? $shipping_id : $this->shipping_id();
		...
		if ($this->flexi_cart_model->update_shipping($shipping_id, $location))
		{
		...
		$this->flexi_cart_model->set_error_message('shipping_update_unsuccessful', 'config');
		return FALSE;
	}
	...
	{
		if ($this->flexi_cart_model->set_manual_shipping($shipping_data))
		{
			if ($this->update_cart_session('shipping_updated_successfully', $recalculate_cart, FALSE))
			{
				return TRUE;
	...
	public function get_shipping_options()
	{
		...
		$shipping_options = $this->flexi_cart_model->shipping_options($this->shipping_location_data());
	...
	public function update_tax($recalculate_cart = TRUE)
	{
		...
		if (! $this->flexi_cart_model->get_enabled_status('tax', TRUE))
		{
			return FALSE;
		}
		...
		if ($this->flexi_cart_model->update_tax($location))
		{
		...
		$this->flexi_cart_model->set_error_message('tax_update_unsuccessful', 'config');
		return FALSE;
	}
	...
	{
		if ($this->flexi_cart_model->set_manual_tax($tax_data))
		{
			if ($this->update_cart_session('tax_updated_successfully', $recalculate_cart, FALSE))
			{
				return TRUE;
	...
	{
		if ($this->flexi_cart_model->set_manual_discount($discount_data))
		{
		...
		$this->flexi_cart_model->set_error_message('discount_update_unsuccessful', 'config');
		return FALSE;
	}
	...
	{
		if ($this->flexi_cart_model->set_discount_codes($discount_codes))
		{
	...
	{
		if ($response = $this->flexi_cart_model->update_discount_codes($discount_codes))
		{
			// Discount codes set.
	...
	{
		if ($this->flexi_cart_model->unset_session_discount($discount_data))
		{
		...
		$this->flexi_cart_model->set_error_message('discount_unset_unsuccessful', 'config');
		return FALSE;
	}
	...
	{
		if ($this->flexi_cart_model->set_surcharge($surcharge_data))
		{
		...
		$this->flexi_cart_model->set_error_message('surcharge_update_unsuccessful', 'config');
		return FALSE;
	}
	...
	{
		if ($this->flexi_cart_model->unset_session_surcharge($surcharge_data))
		{
		...
		$this->flexi_cart_model->set_error_message('surcharge_unset_unsuccessful', 'config');
		return FALSE;
	}
	...
	{	
		...
		if (! $this->flexi_cart_model->get_enabled_status('currency', TRUE))
		{
			return FALSE;
		}
		
		if ($this->flexi_cart_model->update_currency($currency_identifier))
		{
		...
		$this->flexi_cart_model->set_error_message('currency_update_unsuccessful', 'config');
		return FALSE;
	}
	...
	{
		if ($this->flexi_cart_model->set_manual_currency($currency_data))
		{			
			if ($this->update_cart_session('currency_updated_successfully', FALSE, FALSE))
			{
				return TRUE;
	...
	{
		$this->flexi_cart_model->set_minimum_order($value);		
	...
	{
		$this->flexi_cart_model->set_manual_config_setting('display_tax_prices', $tax_status, FALSE);
	...
	{
		$this->flexi_cart_model->set_manual_config_setting('auto_allocate_stock', $allocate_stock_status, FALSE);
	...
	{
		$this->flexi_cart_model->set_manual_config_setting('quantity_limited_by_stock', $stock_limit_quantity_status, FALSE);
	...
	{
		$this->flexi_cart_model->set_manual_config_setting('remove_no_stock_items', $remove_no_stock_status, FALSE);
	...
	{
		$this->flexi_cart_model->set_manual_config_setting('save_banned_shipping_items', $item_shipping_ban_status, FALSE);
	...
	{		
		$this->flexi_cart_model->set_reward_point_multiplier($value);
	...
	{
		$this->flexi->cart_contents['settings']['configuration']['custom_status_1'] = $custom_status;
	...
	{
		$this->flexi->cart_contents['settings']['configuration']['custom_status_2'] = $custom_status;
	...
	{
		$this->flexi->cart_contents['settings']['configuration']['custom_status_3'] = $custom_status;
	...
	private function update_cart_session($status_message, $recalculate_cart = TRUE, $nest_recalc = FALSE, $append_message = TRUE)
	{
		...
		if (! $this->flexi_cart_model->error_messages())
		{
			...
			$response = ($recalculate_cart) ? $this->flexi_cart_model->calculate_cart() : FALSE;
			
			// Check no errors occured during the cart calculation and check whether the '$status_message' should be set.
			if (! $this->flexi_cart_model->error_messages() && (! $nest_recalc || $recalculate_cart))
			{
				$this->flexi_cart_model->set_status_message($status_message, 'config');
			}
	...
	public function empty_cart($reset_shipping = TRUE)
	{
		...
		if ($reset_shipping)
		{
			$this->flexi_cart_model->update_shipping(FALSE, $this->shipping_location_data());
		}
		
		// Reset summary and discount values.
		$this->flexi_cart_model->set_summary_defaults();
		$this->flexi_cart_model->set_discount_defaults();
			
		$this->session->set_userdata(array($this->flexi->cart['name'] => $this->flexi->cart_contents));
		
		$this->flexi_cart_model->set_status_message('cart_emptied', 'config');
		return TRUE;
	}
	...
	{
		$this->flexi->cart_contents = array();
		
		// Unset browser cart session data.
		$this->session->unset_userdata($this->flexi->cart['name']);

		// Reset defaults.
		$this->flexi->cart_contents = $this->flexi_cart_model->set_cart_defaults();

		$this->flexi_cart_model->set_status_message('cart_destroyed', 'config');
		return TRUE;
	}
}
